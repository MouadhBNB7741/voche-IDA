...
export const communityService = {
  ...
  async getPostById(communityId: string, postId: string): Promise<ForumPost> {
    const response = await apiClient.get(COMMUNITY.POST_DETAILS(communityId, postId));
    return response.data;
  },

  async createPost(communityId: string, payload: CreatePostPayload): Promise<ForumPost> {
    const response = await apiClient.post(COMMUNITY.POSTS(communityId), payload);
    return response.data;
  },
  ...
  async getReplies(communityId: string, postId: string): Promise<Comment[]> {
    const response = await apiClient.get(COMMUNITY.REPLIES(communityId, postId));
    return response.data;
  },

  async createReply(communityId: string, postId: string, payload: CreateReplyPayload): Promise<Comment> {
    const response = await apiClient.post(COMMUNITY.REPLIES(communityId, postId), payload);
    return response.data;
  },
  ...
  // Reports
  async reportContent(communityId: string, payload: ReportPayload): Promise<void> {

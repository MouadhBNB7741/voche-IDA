...
/**
 * useSaveTrial
 * Handles save/unsave with:
 * - Optimistic UI update (instant feedback)
 * - Rollback on API failure
 * - React Query cache sync
 * - DataContext sync for savedTrials array
 */
export function useSaveTrial() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ trialId, isSaved }: { trialId: string; isSaved: boolean }) => {
      if (isSaved) {
        // Unsave — DELETE
    ...
    // ✅ Optimistic update — update cache instantly before API responds
    onMutate: async ({ trialId, isSaved }) => {
      ...
      const previousSavedTrials = queryClient.getQueryData<string[]>(['savedTrials']);

      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-3">
            ...
            <Button
              size="lg"
              className={`shadow-lg hover:scale-105 transition-transform font-bold gap-2 ${isRegistered ? "bg-green-600 hover:bg-green-700 text-white border-green-600" : "bg-white text-orange-600 hover:bg-gray-100"}`}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <Card className="p-6 border-border/60 shadow-sm sticky top-24">
            ...
            {isRegistered ? (
              <div className="text-center p-6 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-100 dark:border-green-900/30 mb-4 animate-in zoom-in">
                <div className="w-12 h-12 bg-green-100 dark:bg-green-900/40 rounded-full flex items-center justify-center mx-auto mb-3 text-green-600">
                  <CheckCircle2 size={24} />
                </div>
                <h4 className="font-bold text-green-800 dark:text-green-400 mb-1">You are Registered!</h4>
                <p className="text-xs text-green-700 dark:text-green-500/80">Check your email for access link.</p>
              </div>
            ) : (
          ...
          {relatedEvents.length > 0 && (
            <div className="space-y-4">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Related Events</h3>
              {relatedEvents.map(re => (
                <Card
                  key={re.id}
                  className="p-4 hover:shadow-md transition-all cursor-pointer border-border/60"
                  onClick={() => navigate(`/events/${re.id}`)}
                >
                  <div className="text-xs font-semibold text-accent mb-1">{new Date(re.date).toLocaleDateString()}</div>
                  <h4 className="font-bold text-sm mb-2 line-clamp-2">{re.title}</h4>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <Users size={12} /> {re.participants} attending
                  </div>
                </Card>
              ))}
            </div>
